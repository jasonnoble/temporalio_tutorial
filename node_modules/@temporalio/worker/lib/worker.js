 ...
 */
class CombinedWorkerRunError extends Error {
    constructor(message, { cause }) {
        super(message);
        this.name = 'CombinedWorkerRunError';
        this.cause = cause;
    }
}
...
class Worker {
    ...
    static async createWorkflowCreator(workflowBundle, compiledOptions) {
        ...
        if (compiledOptions.debugMode) {
            if (compiledOptions.reuseV8Context) {
                return await reusable_vm_1.ReusableVMWorkflowCreator.create(workflowBundle, compiledOptions.isolateExecutionTimeoutMs);
            }
            return await vm_1.VMWorkflowCreator.create(workflowBundle, compiledOptions.isolateExecutionTimeoutMs);
        }
        else {
    ...
    static async getOrCreateBundle(compiledOptions, logger) {
        if (compiledOptions.workflowBundle) {
            ...
            if (modules && new Set([...modules, ...bundler_1.defaultWorflowInterceptorModules]).size !== modules.length) {
                    ...
                    return await (0, tracing_1.instrument)(this.tracer, parentSpan, 'workflow.process', async (span) => {
                        ...
                        if (jobs.length === 0) {
                            this.log.trace('Disposing workflow', {
                                ...(state ? (0, workflow_log_interceptor_1.workflowLogAttributes)(state.info) : { runId: activation.runId }),
                            });
                        ...
                        if (state === undefined) {
                            ...
                            if (maybeStartWorkflow !== undefined) {
                                ...
                                this.log.trace('Creating workflow', (0, workflow_log_interceptor_1.workflowLogAttributes)(workflowInfo));
                        ...
                        try {
                            ...
                            this.log.trace('Completed activation', (0, workflow_log_interceptor_1.workflowLogAttributes)(state.info));
     ...
     */
    async processSinkCalls(externalCalls, info, isReplaying) {
        const { sinks } = this.options;
        await Promise.all(externalCalls.map(async ({ ifaceName, fnName, args }) => {
...
async function extractActivityInfo(task, dataConverter, activityNamespace, taskQueue) {
    ...
    return {
        ...
        scheduleToCloseTimeoutMs: (0, time_1.optionalTsToMs)(start.scheduleToCloseTimeout),
    };
}
