...
exports.Runtime = exports.CoreLogger = exports.makeTelemetryFilterString = void 0;
...
class CoreLogger extends logger_1.DefaultLogger {
    constructor(next) {
...
class Runtime {
    constructor(native, options) {
        ...
        this.shouldPollForLogs = new rxjs_1.BehaviorSubject(false);
    ...
    async initLogPolling(logger) {
        ...
        try {
            await (0, rxjs_1.lastValueFrom)((0, rxjs_1.of)(this.shouldPollForLogs).pipe((0, operators_1.map)((subject) => subject.getValue()), (0, operators_1.concatMap)((shouldPoll) => {
                if (!shouldPoll)
                    throw new core_bridge_1.ShutdownError('Poll stop requested');
                return poll(this.native);
            }), (0, operators_1.map)((logs) => {
                ...
                logger.flush();
            }), (0, operators_1.delay)(3), // Don't go wild polling as fast as possible
            (0, operators_1.repeat)()));
        }
        catch (error) {
            // Prevent unhandled rejection
            if (error instanceof core_bridge_1.ShutdownError)
                return;
    ...
    checkHeapSizeLimit() {
        if (process.platform === 'linux') {
            const cgroupMemoryConstraint = Number(this.tryReadFileSync(/* cgroup v2 */ '/sys/fs/cgroup/memory.max') ??
            ...
            if (cgroupMemoryConstraint < os.totalmem() && cgroupMemoryConstraint < v8.getHeapStatistics().heap_size_limit) {
                const totalMemInMb = (0, utils_1.toMB)(os.totalmem(), 0);
                const suggestedOldSpaceSizeInMb = (0, utils_1.toMB)(os.totalmem() * 0.75, 0);
