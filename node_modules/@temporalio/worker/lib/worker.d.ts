 ...
 */
export declare type State = 'INITIALIZED' | 'RUNNING' | 'STOPPED' | 'STOPPING' | 'DRAINING' | 'DRAINED' | 'FAILED';
declare type ExtractToPromise<T> = T extends (err: any, result: infer R) => void ? Promise<R> : never;
declare type Last<T extends any[]> = T extends [...infer _I, infer L] ? L : never;
...
export declare type ActivityTaskWithContext = ContextAware<{
    task: coresdk.activity_task.ActivityTask;
    base64TaskToken: string;
}>;
declare type CompiledWorkerOptionsWithBuildId = CompiledWorkerOptions & {
    buildId: string;
};
/**
...
export declare class CombinedWorkerRunError extends Error {
    readonly name = "CombinedWorkerRunError";
 ...
 */
declare type HeartbeatInput = Heartbeat | ActivityCompleteNotification | HeartbeatFlushNotification | HeartbeatCreateNotification;
export declare type PollerState = 'POLLING' | 'SHUTDOWN' | 'FAILED';
/**
...
export declare class Worker {
     ...
     */
    protected processSinkCalls(externalCalls: SinkCall[], info: WorkflowInfo, isReplaying: boolean): Promise<void>;
    /**
